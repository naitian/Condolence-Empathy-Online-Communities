...
def main(args):
    ...
    outfile = open(
        "./parsed/classified_comments/replies_to_condolence_{}.tsv".format(args.month),
        "w",
    )
    whitelist = set()
    ...
    for line in open(labeled_file, "r"):
        try:
            _, _, _, id, _, _, _, _, _, _, _, score, _ = line.split("\t")
        except:
            continue
        score = float(score)
        if score >= 0.9:
            whitelist.add("t1_{}".format(id))
    ...
    for i, line in enumerate(lzma.open(reddit_file, "rt")):
        ...
        if i % 100000 == 0:
            print(i)
        if comment["parent_id"] in whitelist:
            outfile.write(line)
